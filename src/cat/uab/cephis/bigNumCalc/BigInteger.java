...
public class BigInteger extends Number implements Comparable<BigInteger> {
    ...
    static int modPrime32(BigInteger mod)
    {
        int[] v = mod.mag;
        int inv = -MutableBigInteger.inverseMod32(v[v.length-1]);
        return inv;
    }
    /**
    ...
    public BigInteger(BigInteger r)
    {
        this(r.toByteArray());
    }
    ...
    */
    ...
    public BigInteger modPow(BigInteger exponent, BigInteger m) 
    {
        ...
        if (m.testBit(0)) { // odd modulus
            result = base.oddModPow_ref(exponent, m);
        } else {
            /*
            ...
            BigInteger a1 = (m1.equals(ONE) ? ZERO :
                             base2.oddModPow_ref(exponent, m1));
    ...
    /**
     * Table [0] = x * R mod m
     * Table [1] = (((x * R mod m) ^ 2) * R^-1 mod m) * (x * R mod m) * R^-1 mod m
     * @param y
     * @param z
     * @return 
     */
    BigInteger oddModPow_ref(BigInteger y, BigInteger z) 
    {
        ...
        BigInteger exp = new BigInteger(y);
        BigInteger mod = new BigInteger(z);

        // Select an appropriate window size
        int windowBits = 0;

        while (exp.bitLength() > bnExpModThreshTable[windowBits]) 
        {
            windowBits++;
        }
        ...
        BigInteger[] table = new BigInteger[tblmask];

        // Compute the modular inverse
        int inv = -MutableBigInteger.inverseMod32(mod.mag[mod.getLimbLength()-1]);

//        System.out.println("mprime = " + String.format("%08X", inv));
        
        BigInteger radix = montgomeryRadixFromMod(mod);
...
//                System.out.println("step["+(step++)+"] mr = " + b.toHexString());
     ...
     * @param mod
     * @param inv is -m^-1
     * @return n * R^-1 mod mod
     */
    public BigInteger montReduce(BigInteger mod, int inv) 
    {
//        BigInteger radix = montgomeryRadixFromMod(mod);
        int radixShift =  mod.getLimbLength() * 32;
        BigInteger mprime = new BigInteger(new int[]{0, inv});
        BigInteger b = new BigInteger(new int[]{1, 0});
        
        BigInteger a = this;
        
        for (int i=0; i<mod.getLimbLength(); i++)
        {
            BigInteger ui = a.getLimb(i).multiply(mprime).mod(b);             
//            BigInteger bi = b.pow(i);
//            BigInteger p2 = ui.multiply(mod).multiply(bi);
            BigInteger p2 = ui.multiply(mod).shiftLeft(i*32);
            a = a.add(p2);
        }
        
//        BigInteger bn = b.pow(mod.getLimbLength());
//        a = a.divide(bn);
        a = a.shiftRight(radixShift);
    ...
    /**
     * Montgomery multiplication as 14.36
     * @param y
     * @param mod
     * @param inv
     * @return 
     */
    public BigInteger montMult(BigInteger y, BigInteger mod, int inv)
    {
        BigInteger x = this;
        BigInteger mprime = new BigInteger(new int[]{0, inv});
        BigInteger b = new BigInteger(new int[]{1, 0});
        ...
        int n = mod.getLimbLength();
        
        for (int i=0; i < n; i++)
        {
            BigInteger a0 = a.getLimb(0);
            BigInteger y0 = y.getLimb(0);
            BigInteger xi = x.getLimb(i);
            BigInteger p1 = a0.add(xi.multiply(y0));
            
            BigInteger ui = p1.multiply(mprime).mod(b);
            
            p1 = xi.multiply(y);
            BigInteger p2 = ui.multiply(mod);
            BigInteger num = a.add(p1).add(p2);
    ...
    String toHexString()
    {
        ...
        {
            ret += String.format("%08X ", rev[i] );
        }
        
        return ret;
    }

    private int[] reverse(int[] mag)
    {
        int[] ret = new int[mag.length];
        for (int i=0; i  < ret.length; i++)
            ret[i] = mag[mag.length-1-i];
        
        return ret;
    }

    private int getLimbLength()
    {
        return mag.length;
    }

    static public BigInteger montgomeryRadixFromMod(BigInteger mod)
    {
        BigInteger v = new BigInteger("1");
        
        int shift = mod.getLimbLength() * 32;
        return v.shiftLeft(shift);
    }

    private BigInteger modMult(BigInteger radix, BigInteger mod)
    {
        return multiply(radix).mod(mod);
    }

    private boolean isLessThan(BigInteger mod)
    {
     ...
     * @from https://github.com/an4/Applied-Security-Mod/blob/master/modmul.
     */
     ...
     * hurt in the case k = 1, either.)
     */
    public BigInteger oddModPow(BigInteger y, BigInteger z) {
        // Special case for exponent of one
        if (y.equals(ONE))
            return this;
        ...
        // Convert base to Montgomery form
//        System.out.println("base=" + new BigInteger(1, base).toHexString());
        int[] a = leftShift(base, base.length, modLen << 5);
//        System.out.println("a=" + new BigInteger(1, a).toHexString());
//        System.out.println("mod=" + new BigInteger(1, mod).toHexString());
        ...
        System.out.println("table[0]=" + new BigInteger(1, table[0]).toHexString() );
...
//        System.out.println("square=" + new BigInteger(1, b).toHexString() );
//        System.out.println("mod=" + new BigInteger(1, mod).toHexString() );

        b = montReduce(b, mod, modLen, inv);

//        System.out.println("b=" + new BigInteger(1, b).toHexString() );
...
//            System.out.println("t =" + new BigInteger(1, t).toHexString() );
//            System.out.println("table["+(i-1)+"]=" + new BigInteger(1, table[i-1]).toHexString() );

            int[] prod = multiplyToLen(t, modLen, table[i-1], modLen, null);

//            System.out.println("prod" + new BigInteger(1, prod).toHexString() );

            table[i] = montReduce(prod, mod, modLen, inv);
            
//            System.out.println("mod" + new BigInteger(1, mod).toHexString() );
//
//            System.out.println("table["+i+"]=" + new BigInteger(1, table[i]).toHexString() );
    ...
    boolean isEven()
    {
        return testBit(0) == false;
    }

    private BigInteger getLimb(int i)
    {
        int mlen = mag.length;
        if (i >= mlen)
            return new BigInteger(new int[]{0});
        
        int l = mag[mlen-1-i];
        return new BigInteger(new int[]{0, l});
    }

    private void assertEqual(int buf, int buf2)
    {
        if (buf != buf2)
            throw new RuntimeException("Buf is " + buf + " and buf2 is " + buf2);
    }

    private int getBit(int b)
    {
        return (testBit(b)?1:0);
    }
}